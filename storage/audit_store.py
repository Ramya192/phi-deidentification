...
def _sanitize_audit_log(audit_log: list[dict]) -> list[dict]:
    ...
    Note: this does not add per-record access control to
    GET /records/{id}/audit (there still isn't any) -- seeing this
    sanitized version is a data-exposure fix on its own, but the
    authorization gap on that endpoint is a separate, still-open issue.
    """
    sanitized = []
...
class AuditStore:
    ...
    def store_audit(self, record_id: str, compliance_report: dict[str, Any], audit_log: list[dict]) -> None:
    ...
    def list_audits(self, limit: int = 100, offset: int = 0) -> list[dict[str, Any]]:
        ...
        try:
            rows = (
                ...
                .order_by(AuditRecordDB.persisted_at.desc())
                .limit(limit)
                .offset(offset)
                .all()
            )
